"""
XGBoost para la Ronda 1, con el MISMO criterio de esta ronda: grid search +
seleccion por AUC entre candidatos "calibration-safe" (scale_pos_weight=1),
particion temporal train<2025-01-01 / valid=resto de train.csv, modelo OOT
(metricas honestas) + modelo final reentrenado con 100% de train.csv.
Al final tambien calcula la economia de la politica de aprobacion, igual
que approval_policy.py, para poder agregarla directo a las tablas
comparativas.
"""
import json
import numpy as np
...
from train_models import RAW_FEATURE_COLS, build_preprocessor, load_data, SPLIT_DATE, TARGET
...
def policy_eval(df, p_col):
    ...
    return {
        "n": len(df), "n_aprob": int(mask_ap.sum()), "pct_aprob": float(mask_ap.mean()),
...
def main():
    ...
    out = {"round": 1, "model": "xgboost", "best_cfg": best_cfg, "metrics_oot": metrics_oot,
           "search": search, "policy_valid": pol_valid,
           "ganancia_esperada_test": ganancia_esperada_test, "pct_aprob_test": float(aprob_test.mean())}
    with open(f"{REPORTS_DIR}/xgb_summary_round1.json", "w") as f:
        json.dump(out, f, indent=2)

    print("\n=== RESULTADO XGB RONDA 1 ===")
