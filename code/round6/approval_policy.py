"""
Politica de aprobacion para la Ronda 6 (descartada, calculada por
completitud). El modelo OOT de esta ronda (entrenado jul-dic 2024) SI se
valida contra ene-feb 2025, igual que las Rondas 1/3/4 -- por lo tanto esta
"ganancia real con politica" ES directamente comparable con esas rondas.
"""
import json
import numpy as np
import pandas as pd
...
from train_models import RAW_FEATURE_COLS, clean_inconsistencies, edad_median_valida, TRAIN_START, TRAIN_END
...
def main():
    ...
    va_raw = train[train["fecha_solicitud"] >= TRAIN_END].copy()  # ene-feb 2025, igual que Rondas 1/3/4
    ...
    out = {
        "ronda": 6,
        "nota": "Validacion sobre ene-feb 2025, igual que Rondas 1/3/4 -- comparable directamente",
    ...
    with open(f"{REPORTS_DIR}/policy_summary_round6.json", "w") as f:
        json.dump(out, f, indent=2)

    print(f"RONDA 6 | Validacion (ene-feb 2025): politica=${ganancia_politica:,.0f}  aprobar_todo=${ganancia_aprobar_todo:,.0f}  aprobadas={n_aprob}/{n} ({n_aprob/n*100:.1f}%)  perdida_evitada=${desglose['perdida_evitada']:,.0f}")
    print(f"RONDA 6 | Test.csv (esperado): politica=${ganancia_esperada_politica_test:,.0f}  aprobar_todo=${ganancia_esperada_aprobar_todo_test:,.0f}  aprobadas={n_aprob_test}/{n_test} ({n_aprob_test/n_test*100:.1f}%)")
