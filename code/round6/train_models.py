"""
Ronda 6 de modelamiento — score de probabilidad de default a 12 meses.

Diferencia vs. Ronda 4: se ACORTA la ventana de entrenamiento a los 6 meses
mas recientes antes de la validacion, en vez de usar todo el historial
disponible:

  - Entrenamiento: 2024-07-01 a 2024-12-31 (6 meses, los mas recientes antes
    de la validacion -- NO se usa ene-jun 2024)
  - Validacion:    resto de train.csv = 2025-01-01 en adelante (ene-feb
    2025), EXACTAMENTE la misma ventana de validacion que las Rondas 1, 3 y
    4.

A diferencia de la Ronda 5 (que tambien usaba una ventana de 6 meses pero
CAMBIABA ademas la validacion a jul-dic 2024), aqui la validacion se deja
fija e identica a la Ronda 4 -- el unico cambio es cuanto historial se le
da al modelo para entrenar. Esto permite una comparacion limpia y directa
contra la Ronda 4 para responder la pregunta real: ¿entrenar con menos
datos pero mas recientes generaliza mejor que entrenar con mas datos pero
que incluyen meses mas antiguos (y por lo tanto mas lejanos del regimen de
riesgo actual)?

Se mantienen las mismas 3 correcciones de inconsistencia de la Ronda 4
(edad, ingreso_declarado, tasa_interes_anual vs. TMC). El modelo final que
se usa para scorear test.csv se sigue reentrenando con el 100% de
train.csv (igual que en Rondas 1, 3, 4 y 5) -- la ventana acortada aplica
solo a la seleccion de hiperparametros y a las metricas honestas de este
reporte, no reduce los datos del modelo de produccion.
"""
import json
import numpy as np
import pandas as pd
...
REPORT_DATA_PATH = f"{PROJECT}/reports/round6_report_data.json"

TARGET = "default_12m"
TRAIN_START = pd.Timestamp("2024-07-01")
TRAIN_END = pd.Timestamp("2025-01-01")     # exclusivo: entrena jul-dic 2024 (6 meses)
VALID_END = None                            # sin tope: valida con TODO el resto de train.csv (ene-feb 2025)
...
def main():
    ...
    out = {
        "train_start": str(TRAIN_START.date()), "train_end": str(TRAIN_END.date()), "valid_end": None,
