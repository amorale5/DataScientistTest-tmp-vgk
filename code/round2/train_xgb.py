"""
XGBoost para la Ronda 2 (descartada, calculada por completitud). Mismo
criterio que esa ronda: un unico modelo entrenado SOLO con 2024
(mes_solicitud incluido), usado tanto para validacion (ene-feb 2025) como
para test.csv -- sin reentreno final con el 100% de train.csv.
"""
import json
import numpy as np
import pandas as pd
...
def main():
    ...
    xgb_model = Pipeline([("pre", build_preprocessor()),
                           ("clf", XGBClassifier(objective="binary:logistic", eval_metric="logloss", random_state=42, tree_method="hist", **cfg_clean))])
    xgb_model.fit(Xtr, ytr)
    p_va = xgb_model.predict_proba(Xva)[:, 1]
    metrics_oot = eval_metrics(yva, p_va)
    p_test = xgb_model.predict_proba(Xtest)[:, 1]

    joblib.dump(xgb_model, f"{MODELS_DIR}/xgb_2024_only.joblib")
    ...
    out = {"round": 2, "model": "xgboost", "best_cfg": best_cfg, "metrics_oot": metrics_oot,
           "search": search, "policy_valid": pol_valid,
           "ganancia_esperada_test": ganancia_esperada_test, "pct_aprob_test": float(aprob_test.mean())}
    with open(f"{REPORTS_DIR}/xgb_summary_round2.json", "w") as f:
        json.dump(out, f, indent=2)

    print("\n=== RESULTADO XGB RONDA 2 ===")
