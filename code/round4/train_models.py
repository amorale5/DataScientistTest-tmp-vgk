"""
Ronda 4 de modelamiento — score de probabilidad de default a 12 meses.

Diferencia vs. Ronda 1: se corrigen las 3 inconsistencias definidas por el
usuario (el resto del setup -- imputacion de nulos, exclusiones, particion
temporal, modelo OOT + final -- se mantiene identico a la Ronda 1, para
poder aislar el efecto puro de esta limpieza):

1. edad: consistente en [18, 100]. Fuera de ese rango se marca con un flag
   (flag_edad_inconsistente) y se imputa con la mediana de edades validas
   del set de entrenamiento correspondiente (0 casos en test.csv, 70 en
   train.csv).
2. ingreso_declarado: consistente si es > 200.000 CLP. Valores no nulos
   <=200.000 se marcan con un flag (flag_ingreso_inconsistente) y se
   convierten a NaN -- con lo que quedan sujetos exactamente al mismo
   pipeline de imputacion por mediana-segun-tipo_empleo que ya se usaba
   para los nulos originales (4.573 casos en train.csv, 1.134 en test.csv).
3. tasa_interes_anual: se capa al valor de la Tasa Maxima Convencional (TMC)
   de la CMF vigente para la fecha y tramo de monto de cada solicitud,
   cuando la excede (9 casos en train.csv). Se marca con un flag
   (flag_tasa_corregida). Para fechas de test.csv posteriores al ultimo
   certificado con el que se conto (15-ene a 14-feb-2025), se extrapola el
   valor del ultimo certificado conocido de forma plana (3 casos en
   test.csv) -- es una aproximacion documentada, dado el bajo volumen.
"""
import json
import numpy as np
import pandas as pd
...
MODELS_DIR = f"{PROJECT}/models/round4"
REPORT_DATA_PATH = f"{PROJECT}/reports/round4_report_data.json"
...
def main():
    ...
    tr_win_raw = train[train["fecha_solicitud"] < SPLIT_DATE]
    va_win_raw = train[train["fecha_solicitud"] >= SPLIT_DATE]
