...
de regresion logistica de la Ronda 4 (inconsistencias corregidas). Misma
logica que code/round1/approval_policy.py, con la diferencia de que aqui
hay que aplicar clean_inconsistencies() ANTES de llamar al pipeline, porque
el modelo espera las 3 columnas de flags que genera esa funcion.
"""
import json
import numpy as np
import pandas as pd
...
from train_models import RAW_FEATURE_COLS, clean_inconsistencies, edad_median_valida
...
def main():
    ...
    tr_win_raw = train[train["fecha_solicitud"] < "2025-01-01"]
    va_raw = train[train["fecha_solicitud"] >= "2025-01-01"].copy()

    # mismo criterio que train_models.py: la mediana de edad para el
    # contexto OOT se calcula SOLO sobre la ventana de entrenamiento 2024
    ...
    test_out.to_csv(f"{REPORTS_DIR}/round4_politica_aprobacion_test.csv", index=False)
    ...
    with open(f"{REPORTS_DIR}/policy_report_data_round4.json", "w") as f:
