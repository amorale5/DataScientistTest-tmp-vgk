"""
XGBoost para la Ronda 4, con el MISMO criterio de esta ronda: datos con las
3 inconsistencias corregidas (edad, ingreso, tasa vs TMC), grid search +
seleccion por AUC entre candidatos calibration-safe (scale_pos_weight=1),
particion temporal train<2025-01-01 / valid=resto de train.csv.
"""
import json
import numpy as np
import pandas as pd
...
from train_models import (RAW_FEATURE_COLS, build_preprocessor, load_data, SPLIT_DATE, TARGET,
                           clean_inconsistencies, edad_median_valida)
...
def main():
    ...
    out = {"round": 4, "model": "xgboost", "best_cfg": best_cfg, "metrics_oot": metrics_oot,
           "search": search, "policy_valid": pol_valid,
           "ganancia_esperada_test": ganancia_esperada_test, "pct_aprob_test": float(aprob_test.mean())}
    with open(f"{REPORTS_DIR}/xgb_summary_round4.json", "w") as f:
        json.dump(out, f, indent=2)

    print("\n=== RESULTADO XGB RONDA 4 ===")
