"""
XGBoost para la Ronda 5 (descartada, calculada por completitud). Mismo
criterio: entrena ene-jun 2024, valida jul-dic 2024 (n=20426, NO comparable
en pesos con las demas rondas). Modelo final reentrenado con 100% de
train.csv para test.csv.
"""
import json
import numpy as np
import pandas as pd
...
from train_models import (RAW_FEATURE_COLS, build_preprocessor, load_data, TARGET,
                           clean_inconsistencies, edad_median_valida, TRAIN_START, TRAIN_END, VALID_END)
...
def main():
    ...
    out = {"round": 5, "model": "xgboost", "best_cfg": best_cfg, "metrics_oot": metrics_oot,
           "search": search, "policy_valid": pol_valid,
           "ganancia_esperada_test": ganancia_esperada_test, "pct_aprob_test": float(aprob_test.mean())}
    with open(f"{REPORTS_DIR}/xgb_summary_round5.json", "w") as f:
        json.dump(out, f, indent=2)

    print("\n=== RESULTADO XGB RONDA 5 ===")
