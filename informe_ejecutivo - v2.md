...
Hablar que se usara un enfoque machine learning y que se estimara la probabilidad de que caiga en default

### La regla: umbral de aprobacion por plazo
...
Se recibieron datos de 45.300 solicitudes hisotricas de creditosentre enero 2024 y febrero 2025 junto **con el resultado observado** (si cayeron en default o no) y 12.000 solicitudes nuevas, entre febrero y junio 2025, **sin resultado conocido** — son las que hay que scorear con el modelo final.
...
El trabajo se hizo en dos grandes etapas: primero un **exploratorio de datos** (para entender qué tan confiables son los datos antes de modelar), y después un **proceso iterativo de modelamiento** en 6 rondas, cada una probando una hipótesis concreta.
...
- **Inconsistencias de captura**: 10% de los ingresos declarados están muy por debajo del sueldo mínimo (error de unidades), 70 solicitudes con edad imposible (hasta 133 años), y 9 solicitudes con una tasa de interés que supera la Tasa Máxima Convencional que publica mensualmente la CMF (verificado cruzando la fecha, el monto y el plazo de cada solicitud contra los certificados oficiales).
...
### 4.1 Comparación de modelos — todas las rondas comparables
...
### 4.2 Economía de la política de aprobación, por ronda

Ganancia real (con resultados observados) al aplicar la política de umbrales de la sección 3 sobre las mismas 4.874 solicitudes de validación.
...
*Nota: LightGBM no se evaluó en la política porque su performance fue sistemáticamente igual o peor que la logística en la sección 4.1, y la logística ya se había fijado como modelo preferido por interpretabilidad.*

### 4.3 Elección final: Ronda 4, Regresión Logística
...
## 5. Política de aprobación recomendada y ganancia estimada

Con el modelo de la Ronda 4 y el umbral de la sección 3, **validado con resultados reales de 2025** (4.874 solicitudes nunca vistas al entrenar):
...
Aplicada a las 12.000 solicitudes nuevas de **test.csv**, la ganancia esperada es de **$1.234.680.251**.

---

## 6. Limitaciones y próximos pasos

- El modelo final se reentrenó con el 100% de train.csv (incluyendo enero-febrero 2025) para maximizar los datos disponibles al scorear test.csv; las métricas de la sección 4 provienen de la versión entrenada solo hasta diciembre 2024, para que la validación fuera honesta.
